...
export function buildConfig(env = process.env) {
  ...
  const uploadMaxBytes = Math.max(1_000_000, Number(env.IMAGE_UPLOAD_MAX_BYTES || env.UPLOAD_MAX_BYTES || 5 * 1024 * 1024));
  const videoUploadMaxBytes = Math.max(1_000_000, Number(env.VIDEO_UPLOAD_MAX_BYTES || 50 * 1024 * 1024));
  return {
    port,
